...
The `latex/` directory preserves all **58 original files** from the author's Overleaf `EVRP_Report` source export. No chapter, figure, bibliography entry or license file was edited during extraction. The original 64-page PDF at `Report.pdf` remains unchanged.
...
| `latex/chapters/` | Introduction, background, methodology, evaluation and conclusion |
| `latex/other/` | Title page, abstract, acknowledgements and abbreviations |
| `latex/bibliography.bib` | Original references, using `biblatex` |
| `latex/image/` | Original diagrams, plots and parameter figures |
| `latex/crest.jpg` | Original title-page university crest |
| `latex/LICENSE` | GPL version 3 license text present in the original export |

The entry point is `main.tex` with paths relative to `latex/`. For example, import the contents of that directory into a LaTeX project and select `main.tex` as the main document. The source uses the `report` class, `graphicx`, `biblatex` and mathematical/algorithm packages; bibliography processing requires a compatible LaTeX/Biber environment. The original Overleaf compiler/version configuration was not encoded in this source ZIP. This recovery step did not compile or modify the document, so it does not claim identical output from a newly selected environment.
